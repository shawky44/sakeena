...
class AzkarCategoryScreen extends StatefulWidget {
  ...
  const AzkarCategoryScreen({
    ...
    this.themeColor = const Color(0xFF5F7C7A), this.isCustomAzkar = false,
    this.azkarType, this.onDeleteZikr, this.onReorder,
  });
...
class _AzkarCategoryScreenState extends State<AzkarCategoryScreen> {
  ...
  // ✅ Settings تتحمل مرة واحدة وتتبعت لكل الـ cards
  ...
  Future<void> _loadProgress() async {
    if (widget.azkarType != null) {
      await AzkarStorageService.loadAndApplyProgress(widget.azkarType!, widget.azkarList);
    }
  }

  int  get _completedCount => widget.azkarList.where((z) => z.isCompleted).length;
  bool get _allCompleted   => widget.azkarList.isNotEmpty && widget.azkarList.every((z) => z.isCompleted);

  Future<void> _onCountChanged() async {
    setState(() {});
    if (widget.azkarType != null) {
      await AzkarStorageService.stampToday(widget.azkarType!);
      await AzkarStorageService.saveProgress(widget.azkarType!, widget.azkarList);
  ...
  void _resetAll() {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: const Text('هل تريد إعادة تعيين جميع الأذكار؟', textAlign: TextAlign.right),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: const Text('إلغاء')),
        TextButton(
          onPressed: () async {
            for (var z in widget.azkarList) { z.currentCount = 0; z.isCompleted = false; }
            _completionShown = false;
            if (widget.azkarType != null) await AzkarStorageService.saveProgress(widget.azkarType!, widget.azkarList);
  ...
  void _deleteZikr(int index) {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: const Text('هل تريد حذف هذا الذكر من وردك؟', textAlign: TextAlign.right),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: const Text('إلغاء')),
        TextButton(onPressed: () { Navigator.pop(context); widget.onDeleteZikr?.call(index); setState(() {}); },
          style: TextButton.styleFrom(foregroundColor: Colors.red), child: const Text('حذف')),
      ],
    ));
  }
    ...
    if (_isLoading) {
      return Scaffold(
        ...
        appBar: AppBar(title: Text(widget.title, style: const TextStyle(fontWeight: FontWeight.bold)),
          backgroundColor: widget.themeColor, foregroundColor: Colors.white, elevation: 0),
            ...
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
                ...
                Align(alignment: Alignment.centerLeft,
                  child: Container(height: 34, width: 80,
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(onPressed: _resetAll, icon: const Icon(Icons.refresh_rounded), tooltip: 'إعادة تعيين الكل'),
        ],
      ),
      body: Column(children: [
        Container(height: 6, color: widget.themeColor.withValues(alpha:.2),
          child: LinearProgressIndicator(
            value: widget.azkarList.isEmpty ? 0 : _completedCount / widget.azkarList.length,
        ...
        if (widget.azkarList.isEmpty)
          Expanded(child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Text('لم تقم بإضافة أي أذكار بعد', style: TextStyle(fontSize: 18, color: Colors.grey[600])),
          ])))
        else
          Expanded(
            child: widget.isCustomAzkar
                ? ReorderableListView.builder(
                    ...
                    onReorder: (o, n) { widget.onReorder?.call(o, n); setState(() {}); },
                    proxyDecorator: (child, index, anim) => Material(elevation: 8, borderRadius: BorderRadius.circular(20), child: child),
                    itemBuilder: (context, index) => Dismissible(
                      ...
                      confirmDismiss: (_) async => showDialog<bool>(context: context, builder: (_) => AlertDialog(
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
                        title: const Text('حذف الذكر', textAlign: TextAlign.right),
                        content: const Text('هل تريد حذف هذا الذكر من وردك؟', textAlign: TextAlign.right),
                        actions: [
                      ...
                      child: Stack(children: [
                        AzkarCard(zikr: widget.azkarList[index], onCountChanged: _onCountChanged, fontSize: _fontSize, cardColor: _cardColor),
                        Positioned(top: 16, right: 24,
                          child: Container(padding: const EdgeInsets.all(5),
                            decoration: BoxDecoration(color: widget.themeColor.withValues(alpha:.1), borderRadius: BorderRadius.circular(8)),
                            child: Icon(Icons.drag_handle_rounded, color: widget.themeColor, size: 17))),
                        Positioned(top: 16, left: 24,
                          child: IconButton(onPressed: () => _deleteZikr(index),
                            icon: const Icon(Icons.delete_rounded, color: Color.fromARGB(182,35,30,30)), tooltip: 'حذف')),
                      ]),
                    ),
                  )
                : ListView.builder(
